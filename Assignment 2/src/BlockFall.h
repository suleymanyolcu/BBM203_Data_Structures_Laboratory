...
public:
    ...
    void
    read_blocks(const string &input_file); // Reads the input file and calls the read_block() function for each block;
    void rotateBlockClockwise(const vector<vector<bool>> &src, vector<vector<bool>> &dst);

    bool isCollisionOnEntry(const Block *block, int startRow, int startCol) const;
    ...
    void rotateActiveBlockClockwise();

    void rotateActiveBlockCounterClockwise();
    ...
    void specialHandleRowsForGravity();
};
